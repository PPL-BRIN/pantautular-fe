...
export default function RegisterPage() {
  ...
  const { errors, validateForm, sanitizeInput, getPasswordValidationResult } =
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (id === 'firstName' || id === 'lastName') {
      ...
      setFormData((prev) => ({ ...prev, [id]: sanitizeInput(value) }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Show success popup; backend returns { detail: "A verification email..." }
      setPopup({ visible: true, message: res.detail ?? 'Registrasi berhasil.' });
    } catch (error) {
      setSubmitError(
      ...
      {popup.visible && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
          <div className="bg-white w-full max-w-md p-6 rounded-lg shadow-lg text-center">
            <p className="mb-6 text-gray-800">{popup.message}</p>
            <button
              onClick={handlePopupOk}
              className="px-6 py-2 bg-[#0062E3] text-white rounded-md font-semibold"
            >
              OK
            </button>
          </div>
      ...
      <div className="w-full max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4" data-testid="register-form">
          ...
          <p className="text-sm mt-2">
            Sudah memiliki akun?{' '}
            <a href="/login" className="text-[#0D2B5E] font-semibold">
              Masuk
            </a>
          </p>
